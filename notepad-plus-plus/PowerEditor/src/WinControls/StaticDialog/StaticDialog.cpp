...
[[nodiscard]] static bool shouldModernizeDialogFont(const wchar_t *fontFace)
{
	return fontFace &&
		   (::lstrcmpiW(fontFace, L"MS Shell Dlg") == 0 ||
			::lstrcmpiW(fontFace, L"MS Shell Dlg 2") == 0 ||
			::lstrcmpiW(fontFace, L"MS Sans Serif") == 0);
}
...
[[nodiscard]] static int setFontResource(std::vector<std::byte> &dlgTemplateData, WORD fontSize)
{
	...
	// WORD weight;
	pData = reinterpret_cast<std::byte *>(pointSize + 1);
	pData += sizeof(WORD);
	// BYTE italic;
	pData += sizeof(BYTE);
	// BYTE charset;
	...
	if (shouldModernizeDialogFont(typeFace))
	{
		constexpr wchar_t modernDialogFont[] = L"Segoe UI";
		const size_t currentLen = std::wcslen(typeFace);
		const size_t modernLen = std::size(modernDialogFont) - 1;
		if (modernLen <= currentLen)
		{
			::ZeroMemory(typeFace, (currentLen + 1) * sizeof(WCHAR));
			::CopyMemory(typeFace, modernDialogFont, modernLen * sizeof(WCHAR));
		}
	}
	...
	WORD fontSize)
{
	...
	if (setFontResource(dlgTemplateData, fontSize) < 0)
		return false;
