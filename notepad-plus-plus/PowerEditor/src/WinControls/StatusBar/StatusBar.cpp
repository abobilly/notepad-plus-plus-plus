...
namespace
{
	constexpr int kStatusBarMinHeight = 26;
	constexpr int kStatusBarTextPadding = 8;
	constexpr int kStatusBarDividerInset = 5;

	LOGFONT createStatusBarFont(HWND hwnd)
	{
...
static LRESULT CALLBACK StatusBarSubclass(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	case WM_PRINTCLIENT:
	{
		...
		const int textPadding = DPIManagerV2::scale(kStatusBarTextPadding, dpi);
		const int dividerInset = DPIManagerV2::scale(kStatusBarDividerInset, dpi);

		auto holdPen = static_cast<HPEN>(::SelectObject(hdc, NppDarkMode::getEdgePen()));
		POINT topBorder[] = {
			{rcClient.left, rcClient.top},
			{rcClient.right, rcClient.top}};
		::Polyline(hdc, topBorder, _countof(topBorder));
		...
		for (int i = 0; i < nParts; ++i)
		{
			...
			{
				POINT edges[] = {
					{rcPart.right - std::max(borders.vertical, 1), rcPart.top + dividerInset},
					{rcPart.right - std::max(borders.vertical, 1), rcPart.bottom - dividerInset}};
			...
			rcPart.left += std::max(borders.between, textPadding);
			rcPart.right -= std::max(borders.vertical, std::max(textPadding - 2, 2));
			...
			{
				DrawText(hdc, str.c_str(), static_cast<int>(str.size()), &rcPart, DT_SINGLELINE | DT_VCENTER | DT_LEFT | DT_END_ELLIPSIS);
			}
	...
	case WM_THEMECHANGED:
	{
		...
		LOGFONT lf{createStatusBarFont(hWnd)};
...
void StatusBar::init(HINSTANCE hInst, HWND hPere, int nbParts)
{
	...
	LOGFONT lf{createStatusBarFont(_hSelf)};
	...
	::SendMessage(_hSelf, SB_SETMINHEIGHT, DPIManagerV2::scale(kStatusBarMinHeight, dpi), 0);
