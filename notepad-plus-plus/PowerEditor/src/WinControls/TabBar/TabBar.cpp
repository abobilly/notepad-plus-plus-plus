...
namespace
{
	constexpr COLORREF kNotePadTriplePlusAccent = RGB(84, 122, 181);
	constexpr COLORREF kNotePadTriplePlusAccentMuted = RGB(154, 176, 211);
	constexpr COLORREF kNotePadTriplePlusInactiveText = RGB(101, 108, 118);
	constexpr COLORREF kNotePadTriplePlusInactiveBg = RGB(229, 233, 239);
}

COLORREF TabBarPlus::_activeTextColour = ::GetSysColor(COLOR_BTNTEXT);
COLORREF TabBarPlus::_activeTopBarFocusedColour = kNotePadTriplePlusAccent;
COLORREF TabBarPlus::_activeTopBarUnfocusedColour = kNotePadTriplePlusAccentMuted;
COLORREF TabBarPlus::_inactiveTextColour = kNotePadTriplePlusInactiveText;
COLORREF TabBarPlus::_inactiveBgColour = kNotePadTriplePlusInactiveBg;
...
void TabBar::setFont()
{
	...
	lfVer.lfQuality = CLEARTYPE_NATURAL_QUALITY;

	_hFont = ::CreateFontIndirect(&lf);

	lf.lfWeight = FW_SEMIBOLD;
	static const UINT fontSize = DPIManagerV2::scaleFontForFactor(9);
...
void TabBarPlus::drawItem(DRAWITEMSTRUCT *pDrawItemStruct, bool isDarkMode)
{
	...
	const COLORREF colorActiveBg = isDarkMode ? NppDarkMode::getDlgBackgroundColor() : RGB(248, 250, 252);
	const COLORREF colorInactiveBgBase = isDarkMode ? NppDarkMode::getCtrlBackgroundColor() : RGB(235, 239, 244);
	...
	{
		colorInactiveBg = NppDarkMode::getCtrlBackgroundColor();
	...
	else // inactive tabs
	{
		...
		if (_currentHoverTabItem == nTab && brushColour != colorActiveBg && !_isDragging) // hover on a "darker" inactive tab
		{
			...
			brushColour = hls.toRGB4DarkModeWithTuning(10, 0); // make it lighter slightly
		}
	...
	*out = '\0';
	...
	else
	{
		...
		// 1 space distance to save icon
		rect.left += spaceUnit + _dpiManager.scale(2);
	}

	COLORREF textColor = isSelected ? colorActiveText : colorInactiveText;
	if (!isSelected && _currentHoverTabItem == nTab && !_isDragging)
	{
		textColor = isDarkMode ? NppDarkMode::getTextColor() : colorActiveText;
	}
