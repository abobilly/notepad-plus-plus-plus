...
namespace
{
	bool isLargeToolbar(toolBarStatusType state)
	{
		return state == TB_LARGE || state == TB_LARGE2;
	}

	int getToolbarIconLogicalSize(toolBarStatusType state)
	{
		return isLargeToolbar(state) ? 32 : 16;
	}

	SIZE getToolbarButtonSize(DPIManagerV2 &dpiManager, toolBarStatusType state)
	{
		const int iconSize = dpiManager.scale(getToolbarIconLogicalSize(state));
		const int horizontalChrome = dpiManager.scale(isLargeToolbar(state) ? 12 : 10);
		const int verticalChrome = dpiManager.scale(isLargeToolbar(state) ? 10 : 8);
		return {iconSize + horizontalChrome, iconSize + verticalChrome};
	}

	SIZE getToolbarPadding(DPIManagerV2 &dpiManager, toolBarStatusType state)
	{
		return {
			dpiManager.scale(isLargeToolbar(state) ? 6 : 5),
			dpiManager.scale(isLargeToolbar(state) ? 5 : 4)};
	}
}
...
bool ToolBar::init(HINSTANCE hInst, HWND hPere, toolBarStatusType type, const ToolBarButtonUnit *buttonUnitArray, int arraySize)
{
	...
	int iconSize = _dpiManager.scale(getToolbarIconLogicalSize(_state));
...
void ToolBar::reset(bool create)
{
	...
	const SIZE buttonPadding = getToolbarPadding(_dpiManager, _state);
	const SIZE buttonSize = getToolbarButtonSize(_dpiManager, _state);
	::SendMessage(_hSelf, TB_SETPADDING, 0, MAKELPARAM(buttonPadding.cx, buttonPadding.cy));
	...
	else
	{
		...
		int iconDpiDynamicalSize = _dpiManager.scale(getToolbarIconLogicalSize(TB_SMALL));
	...
	{ // if the toolbar has been recreated, readd the buttons
		...
		::SendMessage(_hSelf, TB_SETBUTTONSIZE, 0, MAKELONG(buttonSize.cx, buttonSize.cy));
