...
LRESULT DockingSplitter::runProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ERASEBKGND:
	{
		...
		::FillRect(hdc, &rc, NppDarkMode::getCtrlBackgroundBrush());
		auto holdPen = static_cast<HPEN>(::SelectObject(hdc, _isLeftButtonDown == TRUE ? NppDarkMode::getHotEdgePen() : NppDarkMode::getEdgePen()));
		...
		if (_flags & DMS_HORIZONTAL)
		{
			...
			POINT splitterLine[] = {
				{rc.left, midY},
				{rc.right, midY}};
			::Polyline(hdc, splitterLine, _countof(splitterLine));
		}
		else
		{
