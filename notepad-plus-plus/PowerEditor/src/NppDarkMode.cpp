...
namespace NppDarkMode
{
	struct Brushes
	{
		...
		Brushes(const Colors &colors)
			: background(::CreateSolidBrush(colors.background)), ctrlBackground(::CreateSolidBrush(colors.softerBackground)), hotBackground(::CreateSolidBrush(colors.hotBackground)), dlgBackground(::CreateSolidBrush(colors.pureBackground)), errorBackground(::CreateSolidBrush(colors.errorBackground))

			  ,
			  edgeBrush(::CreateSolidBrush(colors.edge)), hotEdgeBrush(::CreateSolidBrush(colors.hotEdge)), disabledEdgeBrush(::CreateSolidBrush(colors.disabledEdge))
		{
		}
	...
	struct Pens
	{
		...
		Pens(const Colors &colors)
			: darkerTextPen(::CreatePen(PS_SOLID, 1, colors.darkerText)), edgePen(::CreatePen(PS_SOLID, 1, colors.edge)), hotEdgePen(::CreatePen(PS_SOLID, 1, colors.hotEdge)), disabledEdgePen(::CreatePen(PS_SOLID, 1, colors.disabledEdge))
		{
		}
	...
	static constexpr Colors darkColors{
		HEXRGB(0x242830), // background
		HEXRGB(0x313641), // softerBackground
		HEXRGB(0x3D4552), // hotBackground
		HEXRGB(0x21252C), // pureBackground
		HEXRGB(0xB34747), // errorBackground
		HEXRGB(0xF2F4F7), // textColor
		HEXRGB(0xD7DBE2), // darkerTextColor
		HEXRGB(0x98A1AE), // disabledTextColor
		HEXRGB(0x6CB6FF), // linkTextColor
		HEXRGB(0x4F5866), // edgeColor
		HEXRGB(0x93A6C0), // hotEdgeColor
		HEXRGB(0x39414D)  // disabledEdgeColor
	};

	static constexpr int offsetEdge = HEXRGB(0x101418);
	...
	static void renderButton(HWND hwnd, HDC hdc, HTHEME hTheme, int iPartID, int iStateID)
	{
		...
		dtFlags |= ((nStyle & BS_CENTER) == BS_CENTER) ? DT_CENTER : (nStyle & BS_RIGHT) ? DT_RIGHT
																						 : 0;
		dtFlags |= ((nStyle & BS_VCENTER) == BS_VCENTER) ? DT_VCENTER : (nStyle & BS_BOTTOM) ? DT_BOTTOM
																							 : 0;
	...
	struct ComboboxData
	{
		...
		ComboboxData(LONG_PTR cbStyle)
			: _themeData(VSCLASS_COMBOBOX), _cbStyle(cbStyle)
		{
		}
	...
	struct UpDownData
	{
		...
		explicit UpDownData(HWND hWnd)
			: _isHorizontal((::GetWindowLongPtr(hWnd, GWL_STYLE) & UDS_HORZ) == UDS_HORZ), _isAligned((::GetWindowLongPtr(hWnd, GWL_STYLE) & UDS_ALIGNRIGHT) == UDS_ALIGNRIGHT)
		{
	...
	{
		NppDarkModeParams p{
			g_isAtLeastWindows10 && NppDarkMode::isEnabled() ? L"DarkMode_Explorer" : nullptr, subclass, theme};
		...
		EnumChildWindows(hwndParent, [](HWND hwnd, LPARAM lParam) WINAPI_LAMBDA
			...
			*/
			...
			return TRUE; }, reinterpret_cast<LPARAM>(&p));
	}
	...
	static LRESULT darkToolBarNotifyCustomDraw(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool isPlugin)
	{
		...
		case CDDS_PREPAINT:
		{
			...
			if (NppDarkMode::isEnabled())
			{
				...
				::FillRect(nmtbcd->nmcd.hdc, &nmtbcd->nmcd.rc, NppDarkMode::getCtrlBackgroundBrush());
		...
		case CDDS_ITEMPREPAINT:
		{
			...
			nmtbcd->clrBtnFace = NppDarkMode::getCtrlBackgroundColor();
			nmtbcd->clrBtnHighlight = NppDarkMode::getDlgBackgroundColor();
			...
			{
				NppDarkMode::paintRoundRect(nmtbcd->nmcd.hdc, nmtbcd->nmcd.rc, NppDarkMode::getEdgePen(), NppDarkMode::getDlgBackgroundBrush(), roundCornerValue, roundCornerValue);
	...
	static LRESULT darkRebarNotifyCustomDraw(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool isPlugin)
	{
		...
		case CDDS_PREPAINT:
		{
			...
			::FillRect(lpnmcd->hdc, &lpnmcd->rc, NppDarkMode::getCtrlBackgroundBrush());
			auto holdPen = static_cast<HPEN>(::SelectObject(lpnmcd->hdc, NppDarkMode::getEdgePen()));
			POINT bottomBorder[] = {
				{lpnmcd->rc.left, lpnmcd->rc.bottom - 1},
				{lpnmcd->rc.right, lpnmcd->rc.bottom - 1}};
			::Polyline(lpnmcd->hdc, bottomBorder, _countof(bottomBorder));
			...
			if ((rbBand.fStyle & RBBS_USECHEVRON) == RBBS_USECHEVRON && (rbBand.rcChevronLocation.right - rbBand.rcChevronLocation.left) > 0)
			{
				static int roundCornerValue = 0;
				if (NppDarkMode::isWindows11())
				{
	...
	static void paintMenuBar(HWND hWnd, HDC hdc)
	{
		...
		::FillRect(hdc, &rcBar, NppDarkMode::getCtrlBackgroundBrush());

		auto holdPen = static_cast<HPEN>(::SelectObject(hdc, NppDarkMode::getEdgePen()));
		POINT bottomBorder[] = {
			{rcBar.left, rcBar.bottom - 1},
			{rcBar.right, rcBar.bottom - 1}};
	...
	static void paintMenuBarItems(UAHDRAWMENUITEM &UDMI, const HTHEME &hTheme)
	{
		...
		const UINT dpi = DPIManagerV2::getDpiForWindow(UDMI.dis.hwndItem);
		const int horizontalInset = DPIManagerV2::scale(4, dpi);
		const int verticalInset = DPIManagerV2::scale(2, dpi);
		const int textPadding = DPIManagerV2::scale(8, dpi);
		const int roundCornerValue = NppDarkMode::isWindows11() ? (g_win11CornerRoundness + 1) : 0;

		RECT rcBackground = UDMI.dis.rcItem;
		RECT rcText = UDMI.dis.rcItem;
		::InflateRect(&rcBackground, -horizontalInset, -verticalInset);
		::InflateRect(&rcText, -horizontalInset, -verticalInset);
		rcText.left += textPadding;
		rcText.right -= textPadding;
		...
		else if (((UDMI.dis.itemState & ODS_GRAYED) == ODS_GRAYED) || ((UDMI.dis.itemState & ODS_DISABLED) == ODS_DISABLED) || ((UDMI.dis.itemState & ODS_INACTIVE) == ODS_INACTIVE))
		{
		...
		{
			NppDarkMode::paintRoundRect(UDMI.um.hdc, rcBackground, NppDarkMode::getHotEdgePen(), NppDarkMode::getHotBackgroundBrush(), roundCornerValue, roundCornerValue);
			break;
		}
		...
		{
			NppDarkMode::paintRoundRect(UDMI.um.hdc, rcBackground, NppDarkMode::getEdgePen(), NppDarkMode::getDlgBackgroundBrush(), roundCornerValue, roundCornerValue);
			break;
		}
		...
		case MBI_NORMAL:
		{
			dttopts.crText = NppDarkMode::getDarkerTextColor();
			break;
		}
		...
		case MBI_PUSHED:
		{
			dttopts.crText = NppDarkMode::getAccentColor();
			break;
		}
		...
		::DrawThemeTextEx(hTheme, UDMI.um.hdc, MENU_BARITEM, iTextStateID, buffer.c_str(), static_cast<int>(mii.cch), dwFlags, &rcText, &dttopts);
	}
	...
	static void drawUAHMenuNCBottomLine(HWND hWnd)
	{
		...
		HDC hdc = ::GetWindowDC(hWnd);
		::FillRect(hdc, &rcBar, NppDarkMode::getEdgeBrush());
		::ReleaseDC(hWnd, hdc);
	}
		...
		DWORD_PTR dwRefData)
	{
		...
		case WM_UAHMEASUREMENUITEM:
		{
			...
			const UINT itemHeight = DPIManagerV2::scale(28, dpi);
			pMMI->mis.itemWidth += DPIManagerV2::scale(12, dpi);
			if (pMMI->mis.itemHeight < itemHeight)
			{
				pMMI->mis.itemHeight = itemHeight;
			}
			return 0;
		}
